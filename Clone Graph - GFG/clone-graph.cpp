...
class Solution {
public:
    Node* getAns(Node* node,unordered_map<Node*,Node*>&vis){
        
        
        Node* my=new Node(node->val);
        vis[node]=my;
        for(auto x:node-> neighbors){
            if(vis.find(x)==vis.end()){
                Node* fr=getAns(x,vis);
                (my->neighbors).push_back(fr);
            }
            else{
                (my->neighbors).push_back(vis[x]);
            }
        }
        return my;
    }
